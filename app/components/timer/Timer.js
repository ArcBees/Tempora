...
export default class Timer extends Component {
    ...
    stopTimer() {
        if (!StorageService.getActiveProjectId()) {
            ...
            const worklog = this.buildWorklog();
    ...
    saveSelectedWorklog() {
        const worklog = this.buildWorklogForUpdate();
    ...
    getSecondsFromDuration() {
        const seconds = moment.duration(this.time, 'milliseconds').seconds();
    ...
    getRoundedTime() {
        ...
        if (round > 0) {
            ...
            const rounded = (seconds % round) >= round / 2
                ? parseInt(seconds / round, 10) * round + round
                : parseInt(seconds / round, 10) * round;
    ...
    fetchIssue() {
        IssueService.getIssue(
            this.state.viewType === 'timer' ? StorageService.getActiveIssueId() : StorageService.getWorklogIssueId())
        ...
        return (
            <TimePicker
                ...
                value={moment()
                    ...
                    .add(this.state.duration, 'seconds')}
    ...
    renderButtons() {
        ...
        if (this.state.viewType === 'timer') {
            ...
            buttons.push(<i
                ...
                className="fa fa-floppy-o"
                aria-hidden="true"
                onClick={() => this.saveSelectedWorklog()}
            />);
            buttons.push(<i
                key="btn-timer-delete"
                className="fa fa-trash-o"
                aria-hidden="true"
                onClick={() => this.deleteSelectedWorklog()}
            />);
            buttons.push(<i
                key="btn-timer-show"
            ...
            <div className="timer">
                ...
                <div className="timer__datas">
                    ...
                    <div className="timer__data timer__data--no-highlight">
                        ...
                        <textarea
                            className="timer__data__textarea"
                            rows="2"
                            cols="10"
